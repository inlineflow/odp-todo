...
export function makeSidebar() {
  return new Sidebar({
    ...
    "my-projects": { markup: "My Projects", pList: () => new ProjectList("My Projects", projectRepo.filter(p => p.opts.syntethic === false)) },
    "today": {
  ...
  constructor(navItems) {
    ...
    for (const key in navItems) {
      ...
      if (typeof markup === 'string') {
        const item = new NavItem(markup, []);
        item.pList = value.pList;

        children.push(item);
        // return new NavItem(item, []);
      } else {
        markup.pList = value.pList;
    ...
    children.map(item => {
      item.container.addEventListener("click", (e) => {
        bus.emit("navigation", item.pList());
      });
    });
